...
function Module({ topic, duration, materials, number }) {
  ...
  return (
    <div key={number} className="">
      <header
        className="flex justify-between bg-white px-8 py-3 border-2 border-gray-400"
        onClick={handleState}
      >
        <section className="flex gap-6">
          <section className="w-10 h-10 border-2 rounded-full flex items-center justify-center">
            {number + 1}
          </section>
          <section>
      ...
      <main className={`${currState === "inactive" ? "hidden" : ""}`}>
